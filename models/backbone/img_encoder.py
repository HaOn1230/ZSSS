...
class VPTCLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512, drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False, 
                 num_tokens=50, prompt_dim=512, total_d_layer=11, **kwargs):
        super().__init__()
        self.pretrained = pretrained
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.adaptive_prompt_selection = AdaptivePromptSelection(prompt_dim=self.prompt_dim, num_tokens=self.num_tokens, num_heads=heads)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.total_d_layer >=0:
            prompt_embeddings = self.prompt_proj(self.prompt_embeddings)
            prompt_embeddings = self.prompt_norm(prompt_embeddings)
            prompt_embeddings = self.prompt_dropout(prompt_embeddings).expand(B, -1, -1)
            # Extract spatial features (excluding class embedding and prompt)
            x = torch.cat((x[:, :1, :], prompt_embeddings, x[:, 1:, :]), dim=1)
            spatial_features = x[:, 1 + self.num_tokens:, :]
            spatial_len = spatial_features.shape[1]
            target_H = int(math.sqrt(spatial_len))
            target_W = spatial_len // target_H
            if target_H * target_W != spatial_len:
                raise ValueError(f"Cannot reshape {spatial_len} features into a square grid.")

            try:
                spatial_features = spatial_features.reshape(B, target_H, target_W, -1).permute(0, 3, 1, 2)
            except RuntimeError as e:
                raise RuntimeError(f"Error reshaping spatial_features: {e}")
            adaptive_prompt = self.adaptive_prompt_selection(spatial_features, prompt_embeddings)
            adaptive_prompt = adaptive_prompt.unsqueeze(1)
            x = torch.cat((x[:, :1, :], adaptive_prompt, x[:, 1:, :]), dim=1)
        x = x.permute(1, 0, 2)
        features = []
        outs = []
...
class AdaptivePromptSelection(nn.Module):
    def __init__(self, prompt_dim, num_tokens, num_heads):
        super().__init__()
        self.conv = nn.Conv2d(in_channels=prompt_dim, out_channels=prompt_dim, kernel_size=3, padding=1, stride=1)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(prompt_dim // 2, num_tokens)
        )
    def forward(self, x, prompt_embeddings):
        B, C, H, W = x.shape
        x = self.conv(x).reshape(B, C, -1).permute(0, 2, 1)  
        x = self.norm(x)
        attn_output, _ = self.attn(x, x, x)  
        weights = self.mlp(attn_output.mean(dim=1))  
        weights = F.softmax(weights, dim=-1)  
        adaptive_prompt = (weights.unsqueeze(-1) * prompt_embeddings).sum(dim=1)  
